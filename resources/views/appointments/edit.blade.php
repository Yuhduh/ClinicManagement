    ...
    <section class="px-4 py-6">
        <div class="mx-auto w-full max-w-2xl rounded-[28px] border border-[#d8e0eb] bg-white p-6 shadow-[0_20px_60px_rgba(15,23,42,0.10)] md:p-8">
            <div class="mb-6 flex items-start justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-semibold text-[#172033] md:text-[28px]">Edit Appointment</h1>
                    <p class="mt-1 text-sm text-[#6b7280] md:text-base">Update the appointment details.</p>
                </div>
            ...
            <form method="POST" action="{{ route('appointments.update', $appointment) }}" class="space-y-6">
                ...
                <div class="grid gap-4 md:grid-cols-2">
                    <div>
                        ...
                        <select name="patient_id" class="w-full rounded-xl border border-[#e5e9f1] bg-[#f6f7fb] px-4 py-3 text-sm text-[#6b7280] focus:border-[#2463eb] focus:ring-2 focus:ring-[#d9e7ff]">
                            ...
                            @foreach ($patients as $patient)
                                <option value="{{ $patient->id }}" @selected(old('patient_id', $appointment->patient_id) == $patient->id)>{{ $patient->patient_code }} - {{ $patient->first_name }} {{ $patient->last_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="doctor_id" class="w-full rounded-xl border border-[#e5e9f1] bg-[#f6f7fb] px-4 py-3 text-sm text-[#6b7280] focus:border-[#2463eb] focus:ring-2 focus:ring-[#d9e7ff]">
                            ...
                            @foreach ($doctors as $doctor)
                                <option value="{{ $doctor->id }}" @selected(old('doctor_id', $appointment->doctor_id) == $doctor->id)>{{ $doctor->display_name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-[#172033]">Date</label>
                        <input type="date" name="appointment_date" value="{{ old('appointment_date', $appointment->scheduled_at->format('Y-m-d')) }}" class="w-full rounded-xl border border-[#e5e9f1] bg-[#f6f7fb] px-4 py-3 text-sm text-[#172033] focus:border-[#2463eb] focus:ring-2 focus:ring-[#d9e7ff]" />
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-[#172033]">Time</label>
                        <input type="time" name="appointment_time" value="{{ old('appointment_time', $appointment->scheduled_at->format('H:i')) }}" class="w-full rounded-xl border border-[#e5e9f1] bg-[#f6f7fb] px-4 py-3 text-sm text-[#172033] focus:border-[#2463eb] focus:ring-2 focus:ring-[#d9e7ff]" />
                    </div>
                    <div>
                        <label class="mb-2 block text-sm font-semibold text-[#172033]">Appointment Type</label>
                        <input type="text" name="type" value="{{ old('type', $appointment->type) }}" placeholder="e.g. General Consultation" class="w-full rounded-xl border border-[#e5e9f1] bg-[#f6f7fb] px-4 py-3 text-sm text-[#172033] placeholder:text-[#8a94a6] focus:border-[#2463eb] focus:ring-2 focus:ring-[#d9e7ff]" />
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="notes" rows="4" placeholder="Any additional notes..." class="w-full rounded-xl border border-[#e5e9f1] bg-[#f6f7fb] px-4 py-3 text-sm text-[#172033] placeholder:text-[#8a94a6] focus:border-[#2463eb] focus:ring-2 focus:ring-[#d9e7ff]">{{ old('notes', $appointment->notes) }}</textarea>
                    </div>
                </div>
                ...
                <div class="flex flex-col-reverse gap-3 pt-2 sm:flex-row sm:justify-end">
                    ...
                    <button type="submit" class="rounded-xl bg-[#2463eb] px-5 py-3 text-sm font-medium text-white hover:bg-[#1f54c9]">Update Appointment</button>
                </div>
            </form>
        </div>
    </section>
